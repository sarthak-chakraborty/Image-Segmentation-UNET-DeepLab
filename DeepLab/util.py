...
class SegModel:
    ...
    def create_seg_model(self, net, n=21, backbone='mobilenetv2', load_weights=False, multi_gpu=False):
        ...
        model = Deeplab(weights=None, input_tensor=None, infer=False,
...
#             x = Reshape((self.sz[0]*self.sz[1], -1)) (x)
    ...
    def train(self, model, X, y, val_data, callback_list, tf_board = False, plot_train_process = True):
        h = model.fit(X, y, validation_data = val_data, verbose=1, 
                      batch_size = self.batch_size, epochs = self.epochs, callbacks=callback_list)
        return h
...
class SegmentationGenerator(Sequence):
    ...
    def __init__(self, folder='/workspace/datasets/', mode='train', n_classes=21, batch_size=1, resize_shape=None, 
        ...
        self.image_path_list = sorted(glob.glob(os.path.join(folder, 'train_frames', 'train', '*')))
        self.label_path_list = sorted(glob.glob(os.path.join(folder, 'train_masks', 'train', '*')))
        ...
        if mode == 'test':
            self.image_path_list = sorted(glob.glob(os.path.join(folder, 'val_frames', 'val', '*')))[:100]
